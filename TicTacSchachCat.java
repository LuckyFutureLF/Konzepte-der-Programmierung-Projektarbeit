...
private void erstelleSpielbrett() {
    ...
    private static final Border BORDER_nichtausgewählt = new EmptyBorder(3,3,3,3);
    private static final Border BORDER_ausgewählt = BorderFactory.createCompoundBorder(BorderFactory.createLineBorder(new Color(150,110,40), 2, true), 
...
private void erstelleFigurenPanel() {
    ...
    for (int i = 0; i < Figuren1.length; i++) {
        ...
        spieler1Figuren[i].setBorder(BORDER_nichtausgewählt);
        spieler1Figuren[i].setBorderPainted(true);
    ...
    for (int i = 0; i < Figuren2.length; i++) {
        ...
        spieler2Figuren[i].setBorder(BORDER_nichtausgewählt);
        spieler2Figuren[i].setBorderPainted(true);
...
private void spielfelderGeklickt(int index) {
    if (ausgewählteFigur == null) {
        System.out.println("Bitte zuerst eine Figur auswählen!");
        return;
    }

    //Figur auf das spielBrett
    spielfelder[index].setText(ausgewählteFigur.getText());
    spielfelder[index].setFont(new Font("Dejavu Sans", Font.BOLD, 45));
    spielfelder[index].setForeground(akutellerSpieler == 1 ? Color.WHITE : Color.BLACK);

    //Figur aus dem FigurenPanel entfernen
    ausgewählteFigur.setEnabled(false);

    //Spieler wechseln
    akutellerSpieler = (akutellerSpieler == 1 ? 2 : 1);

    //Figur Auswahl löschen
    ausgewählteFigur.setBorderPainted(false);
